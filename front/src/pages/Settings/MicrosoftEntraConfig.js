...
export class MicrosoftEntraConfig extends React.Component {
  ...
  testConfigWithEmail = async (event) => {
    try {
      event.preventDefault();
      this.props.setIsLoading(true);
      this.setState({ isAuthorizedResult: null, userTeamsResult: null });
      ...
      <div style={{ marginTop: 50 }}>
        ...
        {this.state.testResult != null && (
          <div>
            ...
            <div
              style={{
                backgroundColor: this.state.testResult.userTeams.error ? 'red' : 'green',
                padding: 5,
                color: 'white',
                margin: '5px 0',
              }}
            >
              ...
              <span>
                {this.state.testResult.userTeams.error ||
                  JSON.stringify(this.state.testResult.userTeams.value)}
              </span>
            </div>
          </div>
        )}
